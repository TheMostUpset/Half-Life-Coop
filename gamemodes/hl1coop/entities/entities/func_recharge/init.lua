...
function ENT:UpdateMaxChargeAmount()
	self.MaxCharge = cvars.Number(self.SkillCVar)
end
...
function ENT:Off()
	...
	self:Deny()
	table.Empty(self.ChargerUsersNextUse)
end
...
function ENT:IsUsable(ply)
	return !self.Disabled and ply:Armor() < ply:GetMaxArmor() and self.MaxCharge > 0
end
...
function ENT:Use(activator, caller, useType, value)
	...
	if !self.ChargerUsersNextUse[activator] then
		self.ChargerUsersNextUse[activator] = 0
	end
	if self.ChargerUsersNextUse[activator] <= CurTime() and (activator:IsPlayer() and activator:IsSuitEquipped() or activator:IsWeapon()) then
		if self:IsUsable(activator) then
			...
			self.ChargerUsersNextUse[activator] = CurTime() + self.ChargeDelay
