...
ENT.RestrictedTransitions = {
	...
	{"hls00amrl", "hls00amrltohls01amrl"},
...
function ENT:GetLandmarkName()
	return self.landmark
end

function ENT:GetNextMap()
	return self.MapToChange
end

function ENT:CheckTransition()
	if GAMEMODE:IsCoop() then
		local checkTable = self.RestrictedTransitions
		if MAP.RestrictedTransitions then
			table.Add(checkTable, MAP.RestrictedTransitions)
		end
...
function ENT:CanTrigger(ply)
	...
	if self.EnableDelay and self.EnableDelay > CurTime() then
		if ply and IsValid(ply) then
			ply:ChatPrint("Trigger can be used again in "..math.Round(self.EnableDelay - CurTime(), 1).." seconds")
		end
		return false
	end
	return true
