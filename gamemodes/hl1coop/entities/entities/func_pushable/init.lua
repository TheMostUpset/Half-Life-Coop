...
function ENT:OnTakeDamage(dmginfo)
	...
	if self:HasSpawnFlags(128) then
		// do the damage
		
		if dmginfo:IsDamageType(DMG_BULLET) then
			flDamage = flDamage * GetConVarNumber("func_breakdmg_bullet")
		elseif dmginfo:IsDamageType(DMG_CLUB) then
			flDamage = flDamage * GetConVarNumber("func_breakdmg_club")
		elseif dmginfo:IsDamageType(DMG_BLAST) then
			flDamage = flDamage * GetConVarNumber("func_breakdmg_explosive")
		end
...
function ENT:Die()
	...
	if self.spawnobject then
		...
		if IsValid(dropEnt) then
			dropEnt:SetPos(self:WorldSpaceCenter())
			dropEnt:Spawn()
		end
	end
end
