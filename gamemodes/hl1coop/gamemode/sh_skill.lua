...
function GM:WeaponDamageValuesTable()
	local WeaponDamageValues = {
		...
		["hl1_sk_plr_dmg_mp5_bullet"] = "8", -- old value: 5
...
function GM:CanChangeSkillLevel()
	return true
end
...
function GM:AdjustWallChargers()
	...
	for _, ent in pairs(t) do
		ent:UpdateMaxChargeAmount()
	end
end

function GM:SetSkillLevel(num)
	if !hook.Run("CanChangeSkillLevel") then
		PrintMessage(HUD_PRINTTALK, "Can't set difficulty at the moment!")
		return
	end
	...
	local WeaponDamageValues = hook.Run("WeaponDamageValuesTable")
	if WeaponDamageValues then
		for k, v in pairs(WeaponDamageValues) do
			RunConsoleCommand(k, v)
		end
	end
