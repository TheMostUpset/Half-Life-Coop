...
local cvar_keeprel = CreateConVar("hl1coop_npcrepl_keeprelationship", 1, FCVAR_ARCHIVE, "Replaced NPC inherits relationship to player")
		...
		if cvar and string.len(cvar) > 0 then
			...
			if cvar != ent:GetClass() then
				timer.Simple(0, function()
					if IsValid(ent) then
						local pos, ang, vel, owner, respawnable = ent:GetPos(), ent:GetAngles(), ent:GetVelocity(), ent:GetOwner(), ent.rRespawnable
						ent:Remove()
				...
				timer.Simple(0, function()
					if IsValid(ent) and ent.ReplacedByConVar then
						...
						if IsValid(ent) then
							...
							if cvar_keeprel:GetBool() and rlship then
								if rlship == 1 then
									rlship = "D_HT"
							...
							local customWepTable = hook.Run("HL1_EntReplace_NPCWeapons")
							if customWepTable then
								equipment = customWepTable[ent:GetClass()]
							end
							...
							if equipment then
								if istable(equipment) then
									...
									ent:SetKeyValue("additionalequipment", equipment)
								end
							end
							ent:Spawn()
							ent:Activate()
